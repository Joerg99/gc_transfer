...
nb_digit_threshold = 12000 # number of appearences a characters has to appear in the whole dataset to be kept
...
def extract_anchor_mask(pd_bbox, img_shape):
    ...
    creates a np array of size img_shape with ones. masks bbox position with zeroes on uneven depth level. inversion on even levels
    :param pd_bbox: pandas bounding box labels with left, right, top, bottom, class columns in pixel position
    :param img_shape: shape of reduced input image (e.g. 137 x 102, varies per image)
    :return: numpy array in shape of width, height, depth(2xnb_anchors)
    '''
    np_bbox_anchor_mask = np.ones((img_shape[0], img_shape[1], 2*nb_anchors))
    for index, row in pd_bbox.iterrows():
        if row["class"] > 0:
            # create an array with 0 and 1 representing a bbox mask. depth: class 1 -> 1, class 2 -> 3, class 3 -> 5, class 4 -> 7
            np_bbox_anchor_mask[row["top"]:row["bot"], row["left"]:row["right"], 2*(row["class"]-1)+1] = 0

    for j in range(0, nb_anchors):
        # depth levels 0, 2, 4, 6 is the inversion of levels 1, 3, 5, 7
...
if __name__ == "__main__":
    ...
    outdir_np_cg_unscaled_top_1h = "./data/np_chargrid_unscaled_top_1h/"
    outdir_np_label_unscaled_top_1h = "./data/np_label_unscaled_top_1h/"

    list_filenames = [f for f in os.listdir(outdir_np_cg_unscaled_top) if os.path.isfile(os.path.join(dir_np_chargrid_reduced, f))]
    print(list_filenames)
    ## Load inputs
    tab_img = []
    tab_gt = []
    for i in range(0, len(list_filenames)):
        tab_img.append(np.load(os.path.join(outdir_np_cg_unscaled_top, list_filenames[i])))
        tab_gt.append(np.load(os.path.join(outdir_np_label_unscaled_top, list_filenames[i])))
    ...
    for i in range(0, len(list_filenames)):
        ...
        np.save(os.path.join(outdir_np_cg_unscaled_top_1h, list_filenames[i]), img_1h)
        np.save(os.path.join(outdir_np_label_unscaled_top_1h, list_filenames[i]), gt_1h)
