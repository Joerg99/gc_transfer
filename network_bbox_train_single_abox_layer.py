...
class Network(tf.keras.Model):
    def __init__(self):
        ...
        filters = 1 if no 1hot and only one box per image as a training sample... reduced  version for simplicity
        '''
        self.rpn_cls_out = tf.keras.layers.Conv2D(filters=1, activation="sigmoid" ,kernel_size=1, strides=1, padding="same", kernel_initializer=tf.constant_initializer(value=1e-3), kernel_regularizer=tf.keras.regularizers.l2(l=weight_decay), name="rpn_cls_out")
        self.rpn_cls_out_relu = tf.keras.layers.LeakyReLU(alpha=0.2)
        self.flat1 = tf.keras.layers.Flatten()
        self.dense1 = tf.keras.layers.Dense(units= 12*12, activation="sigmoid")
    ...
    def call(self, input):
        ...
        #out_rpn_cls = self.rpn_cls_out_relu(out_rpn_cls)
        #out_rpn_cls = self.flat1(out_rpn_cls)
        #out_rpn_cls = self.dense1(out_rpn_cls)
        return out_rpn_cls
...
def initialize_network(sample_weight_seg, sample_weight_boxmask):
    ...
    net.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=learning_rate, momentum=momentum, nesterov=False),
                loss=tf.keras.losses.BinaryCrossentropy(), metrics=[tf.keras.metrics.BinaryAccuracy()] ) #loss=tf.keras.losses.BinaryCrossentropy()
...
def train(net):
    ...
    train_img, train_labels_cls = get_data_single_layer(num_samples=8)    #save_folder = f"./output/ss_only/005_dice_1_1_2_2_1weights_allsamples_lrsched_small_model"
    ...
    for epoch in range(epochs):
        ...
        history = net.fit(x=train_img, y=train_labels_cls, shuffle=True, batch_size=batch_size_train,
                          verbose=1, callbacks=[callback], validation_split=0.2) #, class_weight={0:0.1, 1: 10})  #https://github.com/keras-team/keras/issues/8011

        history_loss.append(history.history["loss"])
        history_acc.append(history.history["binary_accuracy"])
        history_val_loss.append(history.history["val_loss"])
        history_val_acc.append(history.history["val_binary_accuracy"])
...
def get_data_single_layer(num_samples):
    import random
    ...
    for example in list_filenames:
        ...
        label_for_im = np.load(f"{dir_to_open}\\{example}")
        img_input.append(im)
        label.append(label_for_im)


    all_samples = []
    for i in range(len(img_input)):
        for j in range(label[i].shape[-1]):
            if np.max(label[i][:, :, j]) != 0.0:
                # toy dataa: label[i][3:8, 3:8, j] = 1
                all_samples.append((img_input[i] , label[i][ :, :, j]))

    #random.shuffle(all_samples)
    all_samples = list(zip(*all_samples))
    train_input = all_samples[0]
    train_labels = all_samples[1]
    train_labels = tf.stack(train_labels[:num_samples])

    # reshape to flatten for dense output:
    # train_labels = tf.reshape(train_labels, [train_labels.shape[0], train_labels.shape[1] * train_labels.shape[2]] )
    # otherwise:
    train_labels = tf.reshape(tf.stack(train_labels[:num_samples]), [ num_samples, 12,12, 1])
    return tf.stack(train_input[:num_samples]), train_labels
...
if __name__ == "__main__":
    ...
    if train_flag:
        ...
        import heapq
        img_input, label = get_data_single_layer(num_samples=4)
        net = initialize_network(None, None)
        net.load_weights("./output/bbox_cls/model_ep_9.ckpt")
        ...
        '''
        play around with weighting loss 
        ...
        bce_loss = bce(y_true,y_pred) #, sample_weight=y_pred[:,:,:,0])
        bce_loss
        y_pred.shape
        '''
    #
    ...
    #     predictions[0,:,:,0]
    # print(heapq.nlargest(4, range(len(np.reshape(predictions[0, :, :, 0], 12 * 12))),
    #                np.reshape(predictions[0, :, :, 0], 12 * 12).take))
    # print(heapq.nlargest(4, range(len(np.reshape(label[0, :, :], 12 * 12))),
    #                np.reshape(label[0, :, :], 12 * 12).take))