...
class Network(tf.keras.Model):
    def __init__(self):
        ...
        self.f3 = tf.keras.layers.Conv2D(filters=4*nb_anchors, kernel_size=3, strides=1, padding="same", kernel_initializer=tf.constant_initializer(value=1e-3), kernel_regularizer=tf.keras.regularizers.l2(l=weight_decay), name="f3_output")
...
def initialize_network(sample_weight_seg, sample_weight_boxmask):
    ...
    # dictionary keys magically correspond to order of y. No idea where the names are coming from
    losses = {'output_1': tf.keras.losses.CategoricalCrossentropy(), 'output_2': tf.keras.losses.CategoricalCrossentropy(),
              'output_3': tf.keras.losses.Huber()}
    sample_weights = {'output_1': sample_weight_seg, 'output_2': sample_weight_boxmask}
    metrics = {'output_1': tf.keras.metrics.CategoricalAccuracy(), 'output_2': tf.keras.metrics.CategoricalAccuracy()}

    net.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=learning_rate, momentum=momentum, nesterov=False),
                loss=losses, sample_weight=sample_weights, metrics=metrics)
...
def train(net, trainset, testset):
    ...
    for epoch in range(epochs):
        ...
        batch_chargrid, batch_seg, batch_mask, batch_coord = extract_batch(trainset, batch_size_train, pad_left_range, pad_top_range, pad_right_range, pad_bot_range)

        history = net.fit(x=batch_chargrid, y=[batch_seg, batch_mask, batch_coord], batch_size= 16,verbose=1, callbacks=[callback])
        ...
        if (epoch % 5) == 0:
            net.save_weights(f"./output/model_ep_{epoch}.ckpt")
...
if __name__ == "__main__":
    ...
    sample_weight_seg, sample_weight_boxmask = get_class_weights() # sample_weight: the weights for how a sample is weighted during metric and loss calculation. Could be None.

    net = initialize_network(sample_weight_seg, sample_weight_boxmask)
    #net.load_weights("./output/model.ckpt")
    plot_subclass_model()

    history_time_train, history_loss, \
    history_loss_output1, history_loss_output2, \
    history_loss_output3, history_time_test, history_val_loss, \
    history_val_loss_output1, history_val_loss_output2, \
    history_val_loss_output3, exec_time = train(net, trainset, testset)
